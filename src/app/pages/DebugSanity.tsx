...
export default function DebugSanity() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-volavan-earth text-volavan-cream p-8">
  ...
  return (
    <div className="min-h-screen bg-volavan-earth text-volavan-cream p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded ${language === 'en' ? 'bg-volavan-aqua text-volavan-earth' : 'bg-volavan-cream/10 text-volavan-cream'}`}
          >
            EN
          </button>
          <button
            onClick={() => setLanguage('es')}
            className={`px-4 py-2 rounded ${language === 'es' ? 'bg-volavan-aqua text-volavan-earth' : 'bg-volavan-cream/10 text-volavan-cream'}`}
          >
            ES
          </button>
        </div>
      ...
      <div className="mb-12">
        <h2 className="text-3xl mb-4 text-volavan-aqua">Programs ({programs.length}) - Language: {language}</h2>
        <div className="space-y-6">
          {programs.length === 0 ? (
            <p className="text-volavan-cream/60">No programs found</p>
          ) : (
            programs.map((program) => (
              <div key={program._id} className="bg-volavan-cream/5 p-6 rounded border border-volavan-cream/10">
                <h3 className="text-2xl mb-2">{program.name}</h3>
                <p className="text-sm text-volavan-cream/60 mb-2">ID: {program._id}</p>
                <p className="text-sm text-volavan-cream/60 mb-4">Slug: {program.slug}</p>
                ...
                {program.editions && (
                  <div className="mt-4 p-4 bg-black/20 rounded">
                    <p className="text-volavan-aqua mb-2">Editions: {program.editions.length}</p>
                ...
                {program.latestEdition && (
                  <div className="mt-4 p-4 bg-black/20 rounded">
                    <p className="text-volavan-aqua mb-2">Latest Edition:</p>
                    ...
                    {program.latestEdition.selectedArtists && (
                      <div className="mt-2">
                        <p className="text-volavan-aqua">Selected Artists ({program.latestEdition.selectedArtists.length}):</p>
                ...
                <details className="mt-4">
                  <summary className="cursor-pointer text-volavan-aqua">View Full Data</summary>
      ...
      <div>
        <h2 className="text-3xl mb-4 text-volavan-aqua">Residencies ({residencies.length})</h2>
        <div className="space-y-6">
          {residencies.length === 0 ? (
            <p className="text-volavan-cream/60">No residencies found</p>
          ) : (
            residencies.map((residency) => (
              <div key={residency._id} className="bg-volavan-cream/5 p-6 rounded border border-volavan-cream/10">
                <h3 className="text-2xl mb-2">{residency.title}</h3>
                <p className="text-sm text-volavan-cream/60 mb-4">Slug: <span className="text-volavan-aqua font-bold">{residency.slug}</span></p>
                <p className="text-sm text-volavan-cream/60 mb-2">Link: <a href={`/residencies/${residency.slug}`} className="text-volavan-aqua underline">/residencies/{residency.slug}</a></p>
                ...
                {residency.artists && residency.artists.length > 0 && (
                  <div className="mt-4 p-4 bg-black/20 rounded">
                    <p className="text-volavan-aqua mb-2">Artists ({residency.artists.length}):</p>
                    <ul className="ml-4 list-disc">
      ...
      {/* Artists Section */}
      <div>
        <h2 className="text-3xl mb-4 text-volavan-aqua">Artists (From Residencies)</h2>
        <div className="space-y-6">
          {allArtists.length === 0 ? (
            <p className="text-volavan-cream/60">No artists found</p>
          ) : (
            allArtists.map((artist) => (
              <div key={artist._id} className="bg-volavan-cream/5 p-6 rounded border border-volavan-cream/10">
                <h3 className="text-2xl mb-2">{artist.name}</h3>
                <p className="text-sm text-volavan-cream/60 mb-4">Slug: <span className="text-volavan-aqua font-bold">{artist.slug}</span></p>
                <p className="text-sm text-volavan-cream/60 mb-2">Link: <a href={`/artists/${artist.slug}`} className="text-volavan-aqua underline">/artists/{artist.slug}</a></p>
